...
describe("server-side game scoring", () => {
  ...
  it("scores Ledger from canonical card costs and the Blue perspective", () => {
    ...
    // Red spent two more, so Blue owns a +2 advantage.
    expect(scoreRun(challenge, { answers: [{ ...base, guess: 2 }] }, 0)).toBe(
      100,
    );
    expect(scoreRun(challenge, { answers: [{ ...base, guess: -2 }] }, 0)).toBe(
      0,
  ...
  it("rejects Ledger sequences that escape the signed pool or answer range", () => {
    const blue = allCards.find((card) => card.elixir === 3)!;
    const red = allCards.find((card) => card.elixir === 5)!;
    const outsider = allCards.find(
      (card) => card.id !== blue.id && card.id !== red.id,
    )!;
    const challenge = {
      mode: "practice" as const,
      practiceKind: "ledger" as const,
    ...
    expect(() => scoreRun(challenge, { answers: [answer] }, 0)).toThrow(
      /not from the signed deck/,
    );
    ...
    ).toThrow(/Ledger answer is invalid/);
  });
