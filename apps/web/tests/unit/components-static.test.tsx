...
describe('standalone pages', () => {
  ...
  it('keeps discovery, support, policy, and setup copy in real HTML', () => {
    ...
    expect(xp).toContain('Battle Tag +100')
    ...
    expect(updates).toContain('Three community badges enter the arena')
    expect(updates).toContain('Log claims the Season 135 Free Pass')
    expect(updates).toContain('A royal sweep closes Season 135')
    expect(updates).toContain('<strong>Battle Tag</strong>')
    expect(updates).toContain('<a href="/feed.xml">Follow via RSS</a>')
    expect(updates).toContain('<section id="community-badges-enter-the-arena"')
    expect(updates).toContain('<section id="season-135-free-pass-recipient"')
    expect(updates).toContain('<section id="season-135-results-season-136-opening"')
