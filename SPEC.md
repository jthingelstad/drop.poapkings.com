...
- `apps/web/src/components/shell/DesktopWallpaper.tsx` - persistent Falling
  ...
  scene defaults off; its control cycles off -> subtle -> background -> full
  screen -> off. Subtle runs the same live canvas at lower opacity. Full screen
  hides the panels over that persistent canvas instead of creating a second
  scene; its dismissing input restores the panels and shuts the cards off.
  Reduced motion freezes the composition while retaining off, subtle, and
  background.
...
Each earned badge rung likewise has one permanent address:
`/share/{playerTag}/badge/{badgeSlug}/{rung}`. `rung` is one-based in the public
URL while the internal snapshot retains its zero-based index. `POST
/badges/{badgeSlug}/share` is owner-only, verifies the requested milestone
against server-owned badge counters and timestamps, and freezes the public
...
operator tool still validates the retained run and owner before any repair.
Live API configuration, account data, and
