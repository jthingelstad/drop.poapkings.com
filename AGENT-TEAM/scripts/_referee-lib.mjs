// Shared conventions + helpers for the read-only Fair Play Referee scripts.
//
// These scripts are a documented, sanitized read surface over the Drop game
// table. They import NOTHING from services/api (workspace-boundary rule); the
// key conventions below are duplicated here on purpose and versioned by the
// evidence item's `schemaVersion`.
//
...
// is expected to have assumed the read-only RefereeReadRole). Fails closed if no
// region is configured.
export function client() {
  if (cachedDoc) return cachedDoc;
  const region = process.env.AWS_REGION || process.env.AWS_DEFAULT_REGION;
  if (!region) failClosed("no_aws_region", "Set AWS_REGION for the referee host");
...
export { GetCommand, QueryCommand, ScanCommand };
