...
def prompt(entry: dict, repo: Path = REPO) -> str:
    if "prompt" in entry:
        return entry["prompt"]
    paths = [repo / "AGENT-TEAM/WORKFLOW.md", repo / "AGENT-TEAM/README.md", repo / entry["objective_file"]]
    rendered = ", ".join(f"`{path}`" for path in paths)
    return (
        f"Follow the project instructions already loaded by Codex. Read {rendered} completely, "
        f"then pursue the `{entry['objective']}` objective exactly as written. Measure current "
        "evidence before changing anything. Own a clear gap through source fix, regression "
        "coverage, verification, deployment or restart when required, and natural acceptance. "
        "Use issues only for multi-run work, external blockers, durable audits, or Jamie decisions. "
        "Run preflight first; acquire the objective lease only before mutation; never publish "
        "pre-existing work; preserve human and privacy boundaries; and end with a clean repository. "
        "Keep automation memory to Current state, Active watches, and one replace-in-place Latest run."
    )
...
def validate(plan: dict, repo: Path = REPO) -> list[str]:
    ...
    for entry in entries:
        if entry.get("schedule_of"):
            ...
            if (not owner or owner.get("schedule_of")
...
def audit(plan: dict, *, codex_home: Path = CODEX_HOME, repo: Path = REPO) -> tuple[list[str], list[str]]:
    ...
    for entry in plan["automation"]:
        ...
        if drift:
            ...
            successes.append(f"OK  {entry['id']}  {entry['objective']}  {entry['status']}  {entry['rrule']}")
    return successes, failures


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--registry-only", action="store_true", help="skip live Codex automation comparison")
    ...
    if args.registry_only:
        ...
        if not failures:
            successes.append(f"OK  registry  {len({entry['objective'] for entry in plan['automation']})} objective owners")
    else:
