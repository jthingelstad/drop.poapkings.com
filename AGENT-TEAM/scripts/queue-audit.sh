...
printf '%s' "$issues" | jq -r '.[] | select([.labels[].name] | index("decision")) | "  #\(.number)  \(.title)"'
...
old="$(printf '%s' "$issues" | jq -r --argjson retired "$retired" '
  ...
  | select($controls | length > 0) | "  #\(.number)  [\($controls | join(","))]  \(.title)"')"
...
if [ "$verdict" -eq 0 ]; then echo "  ✓ every open issue has one known objective owner and no retired controls"; fi
exit "$verdict"
