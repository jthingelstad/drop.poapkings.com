...
Player Updates use a dedicated bearer token generated by `npm run bootstrap:aws`
and retained in the root mode-0600 `.env`. Deployment passes it to the API
Lambda through the NoEcho `UpdatesPublishToken` parameter. The AGENT-TEAM and
Control Room send that token directly to `POST /admin/updates` over HTTPS, so
routine publication has no AWS profile, STS session, or interactive login.
