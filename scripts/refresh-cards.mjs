...
// diffs against src/data/cards.json, commits + pushes if changed.
//
// Usage:
...
const dotenv = await readFile(join(ROOT, '.env'), 'utf8').catch(() => '')
...
console.log(`\nWrote src/data/cards.json (${candidate.count} cards, ${today})`)
...
execSync('git add src/data/cards.json', { cwd: ROOT, stdio: 'inherit' })
